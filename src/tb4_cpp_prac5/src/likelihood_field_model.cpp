    ...
    LikelihoodFieldModel::sensorFunction(nav2_amcl::LaserData * data, pf_sample_set_t * set)
    {
        ...
        for (j = 0; j < set->sample_count; j++) {
                ...
                */
                ...
                pose_lidar.v[0] = pose_robot.v[0] + lidar_pose_to_robot.v[0] * cos(pose_robot.v[2]) - lidar_pose_to_robot.v[1] * sin(pose_robot.v[2]);
                pose_lidar.v[1] = pose_robot.v[1] + lidar_pose_to_robot.v[0] * cos(pose_robot.v[2]) - lidar_pose_to_robot.v[1] * sin(pose_robot.v[2]);
                pose_lidar.v[2] = pose_robot.v[2] + lidar_pose_to_robot.v[2];
                p = 1.0;
                ...
                for (i = 0; i < data->range_count; i += step) {
                    ...
                    */
                        ...
                        hit_x = pose_lidar.v[0] + obs_range * cos(pose_lidar.v[2] + obs_bearing);
                        hit_y = pose_lidar.v[1] + obs_range * sin(pose_lidar.v[2] + obs_bearing);

                        int mi = MAP_GXWX(self->map_, hit_x);
                        int mj = MAP_GYWY(self->map_, hit_y);

                        if (MAP_VALID(self->map_, mi, mj)) {
                            int index = MAP_INDEX(self->map_, mi, mj);
