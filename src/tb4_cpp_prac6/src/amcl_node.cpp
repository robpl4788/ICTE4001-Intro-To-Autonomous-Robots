...
AmclNode::pf_resample(pf_t * pf){
  ...
  */
  ...
  for (int s = 0; s < old_particle_set->sample_count; s++) {
    pf_sample_t *sample = &old_particle_set->samples[s];
    cd_old_set[s + 1] = sample->weight + cd_old_set[s];
  }
  ...
  while(new_particle_set->sample_count < pf->max_samples)
  {
    ...
    */
    ...
    if (k <= 1) {
      ...
      double a =  (k-1.0) / (2.0 * pf_err_);
      double b = 2.0 / (9.0 * (k-1.0));
      double c = std::sqrt(2.0 / (9.0 * (k-1.0))) * pf_z_;
      M = std::max(min_particles_, static_cast<int>(a * std::pow(1 - b + c, 3)));
    }
  ...
  */
  ...
  for (int s = 0; s < new_particle_set->sample_count; s++) {
    pf_sample_t *sample = &new_particle_set->samples[s];
